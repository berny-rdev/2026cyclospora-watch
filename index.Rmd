...
required_pkgs <- c("googlesheets4", "dplyr", "tidyr", "stringr", "lubridate",
                    "ggplot2", "janitor", "purrr", "knitr", "DT", "httr2", "jsonlite")
...
call_claude_classify_dynamic <- function(items, known_categories, api_key = Sys.getenv("ANTHROPIC_API_KEY"),
  ...
  prompt <- paste0(
    ...
    "For each item below:\n",
    "1. If it clearly matches an EXISTING category, use that EXACT category name - don't ",
    "create a near-duplicate or synonym.\n",
    "2. If it genuinely doesn't fit any existing category, invent ONE new concise category ",
    "name in lowercase_snake_case (1-3 words) that could apply to future similar items too. ",
    "Do NOT use \"other\" - always pick or create a real, specific category.\n",
...
produce_signal <- produce_freq %>%
  ...
  # estimate - discounts small-n flukes, rewards foods elevated even
  # under the pessimistic estimate.
...
tracker built by a very fed up public health enthusiast trying to spot patterns
...
produce_signal %>%
  ...
  coord_flip() +
  labs(title = "Produce Signal Ratio", x = NULL, y = "Signal ratio (case % / baseline %)") +
