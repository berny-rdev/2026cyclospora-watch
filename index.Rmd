...
## Cases by Canadian province

```{r canada-map, fig.width=9, fig.height=7}
if ("state" %in% names(df) && requireNamespace("mapcan", quietly = TRUE)) {
...
if (!is.null(tally_data) && nrow(tally_data) > 0) {
  ...
  plot_data <- tally_data %>%
    filter(food %in% top_foods) %>%
    mutate(food = factor(food, levels = top_foods))

  ggplot(plot_data, aes(x = food, y = mentions, fill = restaurant)) +
